...
if (isMarketplaceInstall) {
  ...
  // Remove stale .bundle.mjs files from ~/.claude/hooks/<Handler>/.
  // Generated by an older plugin system version; left behind when the hooks
  // field was temporarily removed from plugin.json (commit 9a7c60c).
  // Auto-discovered by Claude Code with #!/usr/bin/env bun shebang, but bun
  // is not in the harness execution PATH → exit 127 on every session start.
  const STALE_BUNDLES = LTM_HOOK_PATTERNS.map(p => {
    const name = basename(p, ".ts");
    return join(CLAUDE_DIR, "hooks", name, `${name}.bundle.mjs`);
  });
  let bundlesRemoved = 0;
  for (const p of STALE_BUNDLES) {
    try { rmSync(p); bundlesRemoved++; } catch {}
  }
  if (bundlesRemoved > 0)
